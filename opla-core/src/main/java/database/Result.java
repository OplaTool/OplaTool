...
public class Result {
	...
	public AllMetrics getMetrics(List<FunResults> funResults, List<Solution> list, Execution execution,
		...
		for (int i = 0; i < list.size(); i++) {
			for (int j = 0; j < numberOfVariables; j++) {
				...
				if (objectiveFuncs.contains("h"))
					allMetrics.getH()
							.add(buildHMetrics(idSolution, execution, experiement, metrics, arch));
	...
	private H buildHMetrics(String idSolution, Execution execution, Experiment experiement,
			MetricsEvaluation metrics, Architecture arch) {

		H h = new H(idSolution, execution, experiement);

		h.setCohesion(metrics.evaluateCohesion(arch));
